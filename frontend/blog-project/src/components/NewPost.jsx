...
const NewPost = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    // Check for user authentication and required fields
    if (!user || !user.token) {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => {
        // Redirect to the user's posts page after a successful post
        navigate('/my-posts');
      }, 1500);
    ...
    <div className="min-h-screen bg-gray-100 font-sans text-gray-800 p-4 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center">
          <span className="text-xl font-semibold">New Post</span>
        </div>
        <div className="w-8" />
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg p-6 mb-8 space-y-6">
        ...
        {success && (
          <p className="text-green-500 text-center text-sm">Post created successfully!</p>
        )}
        <button
          type="submit"
        ...
        <button
          ...
          className="w-full py-4 px-6 rounded-full text-blue-600 font-semibold shadow-md border border-blue-600 hover:bg-blue-50 transition-colors mt-2"
        >
          My posts
        </button>
      </form>
